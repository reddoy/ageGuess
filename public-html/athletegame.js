/**
 * Athlete Game Mode Handler
 * Handles the athlete age guessing game logic
 */
...
async function checkGuess(event) {
  ...
  try {
    const response = await fetch('/check/guess/athlete', {
      method: 'POST',
      headers: {
...
/**
 * Fetches a new random athlete image
 */
async function getNewImage() {
  try {
    const response = await fetch('/get/image/athlete');
    const data = await response.json();
